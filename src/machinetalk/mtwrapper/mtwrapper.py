...
from locking import ObjectLock, AccessError
...
logger = logging.getLogger('mtwrapper')
...
class LinuxCNCWrapper(object):
    def __init__(
        ...
        ping_interval=2,
        no_tool_table=False,
        no_command_channel=False,
        no_error_channel=False,
    ):
        self.host = host
        ...
        self.running = False
        self._tool_table_enabled = not no_tool_table
        self._command_enabled = not no_command_channel
        self._error_enabled = not no_error_channel
        ...
        # Linuxcnc
        try:
            self.stat = ObjectLock(linuxcnc.stat(), 'linuxcnc.stat', MAX_LOCK_TIME)
            if self._command_enabled:
                self.command = ObjectLock(
                    linuxcnc.command(), 'linuxcnc.command', MAX_LOCK_TIME
                )
            if self._error_enabled:
                self.error = ObjectLock(
                    linuxcnc.error_channel(), 'linuxcnc.error_channel', MAX_LOCK_TIME
                )
            self._is_joint_axis = hasattr(self.stat, 'joints')
            ...
            if open_file != "" and self._command_enabled:
                ...
                logger.debug("loading default file {}".format(open_file))
    ...
    def process_sockets(self):
        ...
        while not self.shutdown.is_set():
            ...
            if (
                self._error_enabled
                and self.error_socket in s
                and s[self.error_socket] == zmq.POLLIN
            ):
                self.process_error(self.error_socket)
            if (
                self._command_enabled
                and self.command_socket in s
                and s[self.command_socket] == zmq.POLLIN
            ):
            ...
            try:
                if self.status_service_subscribed:
                    ...
                    if self.ping_count == self.ping_ratio:
                        self.ping_status()
                if self._error_enabled and self.error_service_subscribed:
                ...
                logger.error('locking timed out after %.2f s' % MAX_LOCK_TIME)
                continue
    ...
    def update_config(self, stat):
        ...
        if self.config_first_run:
            ...
            self.status.config.extrajoints = 0
            self.status.config.spindles = 0
        ...
        if self._is_joint_axis:  # post joint-axis merge
            for name in ['joints', 'spindles']:
                modified |= self.update_config_value(name, getattr(stat, name))
            modified |= self.update_config_value('extrajoints', stat.num_extrajoints)
        ...
        if self._is_joint_axis:  # post joint-axis merge
            tx_joint = EmcStatusConfigJoint()
            for index, stat_joint in enumerate(stat.joint):
                tx_joint.Clear()
                ...
                if len(self.status.config.joint) == index:
                    ...
                    joint = self.status.config.joint[index]
                    joint.index = index
                    joint.joint_type = EMC_JOINT_LINEAR
                    joint.backlash = 0.0
                    joint.max_ferror = 0.0
                    joint.max_position_limit = 0.0
                    joint.min_ferror = 0.0
                    joint.min_position_limit = 0.0

                joint = self.status.config.joint[index]
                joint_modified |= self.update_proto_value(
                    joint, tx_joint, 'joint_type', stat_joint['jointType']
                )
                ...
                for name in [
                    ...
                    joint_modified |= self.update_proto_float(
                        joint, tx_joint, name, stat_joint[name]
                    )

                if joint_modified:
                    tx_joint.index = index
                    self.status_tx.config.joint.add().CopyFrom(tx_joint)
                    modified = True
    ...
    def update_motion(self, stat):
        ...
        if not self._is_joint_axis:
            ...
            for name in ['spindle_speed', 'spindlerate']:
                modified |= self.update_motion_float(name, getattr(stat, name))
        ...
        for index, stat_axis in enumerate(stat.axis):
            ...
            axis_modified |= self.update_proto_float(
                axis, tx_axis, 'velocity', stat_axis['velocity']
            )
        ...
        if self._is_joint_axis:  # post joint-axis merge
            ...
            for index, stat_joint in enumerate(stat.joint):
                ...
                if len(self.status.motion.joint) == index:
                    ...
                    joint = self.status.motion.joint[index]
                    joint.index = index
                    joint.enabled = False
                    joint.fault = False
                    joint.ferror_current = 0.0
                    joint.ferror_highmark = 0.0
                    joint.homed = False
                    joint.homing = False
                    joint.inpos = False
                    joint.input = 0.0
                    joint.max_hard_limit = False
                    joint.max_soft_limit = False
                    joint.min_hard_limit = False
                    joint.min_soft_limit = False
                    joint.output = 0.0
                    joint.override_limits = False
                    joint.velocity = 0.0
                ...
                if joint_modified:
                    tx_joint.index = index
                    self.status_tx.motion.joint.add().CopyFrom(tx_joint)
                    modified |= True
            del tx_joint

        if self._is_joint_axis:
            tx_spindle = EmcStatusMotionSpindle()
            for index, stat_spindle in enumerate(stat.spindle):
                tx_spindle.Clear()
                spindle_modified = False

                if index == stat.spindles:
                    break

                if len(self.status.motion.spindle) == index:
                    self.status.motion.spindle.add()
                    spindle = self.status.motion.spindle[index]
                    spindle.index = index
                    spindle.brake = False
                    spindle.direction = 0
                    spindle.enabled = False
                    spindle.override_enabled = False
                    spindle.speed = 0.0
                    spindle.homed = False
                    spindle.orient_fault = False
                    spindle.orient_state = False
                    spindle.increasing = False
                    spindle.override = 0.0
                ...
                for name in [
                    ...
                    spindle_modified |= self.update_proto_value(
                        spindle, tx_spindle, name, stat_spindle[name]
                    )

                for name in ['speed', 'override']:
                    spindle_modified |= self.update_proto_float(
                        spindle, tx_spindle, name, stat_spindle[name]
                    )

                if spindle_modified:
                    tx_spindle.index = index
                    self.status_tx.motion.spindle.add().CopyFrom(tx_spindle)
                    modified |= True
            del tx_spindle
    ...
    def process_status(self, zmq_socket):
        try:
            ...
            logger.debug("process status called {} {}".format(subscription, status))
            logger.debug(
                "status service subscribed: {}".format(self.status_service_subscribed)
            )

        except zmq.ZMQError as e:
            logger.error('ZMQ error: {}'.format(e))
    ...
    def process_error(self, zmq_socket):
        try:
            ...
            logger.debug("process error called {} {}".format(subscription, status))
            logger.debug(
    ...
    def command_completion_thread(self, identity, ticket):
        ...
        logger.debug('command #{} from {} completed'.format(ticket, identity))
    ...
    def process_command(self, zmq_socket):
        ...
        logger.debug("process command called, id: {}".format(identity))
        ...
        try:
            if self.rx.type == MT_PING:
                ...
                if self.rx.HasField(
                    ...
                    axis = self.rx.emc_command_params.index
                    if self._is_joint_axis:
                        if self.rx.emc_command_params.HasField('enable'):
                            jjogmode = self.rx.emc_command_params.enable
                        else:
                            jjogmode = False
                        self.command.jog(linuxcnc.JOG_STOP, jjogmode, axis)
                    else:
                ...
                if (
                    ...
                    if self._is_joint_axis:
                        if self.rx.emc_command_params.HasField('enable'):
                            ...
                            jjogmode = False
                        self.command.jog(linuxcnc.JOG_CONTINUOUS, jjogmode, axis, velocity)
                    else:
                ...
                if (
                    ...
                    if self._is_joint_axis:
                        if self.rx.emc_command_params.HasField('enable'):
                            ...
                            jjogmode = False
                        self.command.jog(linuxcnc.JOG_INCREMENT, jjogmode, axis, velocity, distance)
                    else:
...
def main():
    parser = argparse.ArgumentParser(
        description='Mtwrapper is wrapper around the linuxcnc python module'
    ...
    parser.add_argument(
        '-nt',
        '--no-tool-table',
        help='Disable tool table messages',
        action='store_true',
    )
    parser.add_argument(
        '-nc',
        '--no-command-channel',
        help='Disable the command channel.',
        action='store_true',
    )
    parser.add_argument(
        '-ne',
        '--no-error-channel',
        help='Disable the error channel.',
        action='store_true',
    )
    ...
    logger.debug("announcing mtwrapper")
    ...
    try:
        ...
        mtwrapper = LinuxCNCWrapper(
            context,
            host=hostname,
            loopback=(not remote),
            ini_file=ini_file,
            svc_uuid=uuid,
            no_tool_table=args.no_tool_table,
            no_command_channel=args.no_command_channel,
            no_error_channel=args.no_error_channel,
        )

        while mtwrapper.running and not check_exit():
            time.sleep(1)
    except Exception as e:
        logger.error("uncaught exception: " + str(e))
