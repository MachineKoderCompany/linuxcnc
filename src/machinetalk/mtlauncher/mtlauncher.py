...
class Mtlauncher(object):
    def __init__(
        ...
        ping_interval=2.0,
        loopback=False,
        config_dir='~/.config/machinetalk/mtlauncher',
    ):
    ...
    def _create_services(self, host_in_name, svc_uuid):
        if self.name is None:
            self.name = 'Machinetalk Launcher'
        if host_in_name:
...
def main():
    parser = argparse.ArgumentParser(
        description='mtlauncher is Machinetalk based session/configuration launcher for Machinetalk'
    )
    parser.add_argument(
        '-n', '--name', help='Name of the machine', default="Machinetalk Launcher"
    )
    parser.add_argument(
        '-s',
        '--suppress_ip',
    ...
    while mtlauncher.running and not check_exit():
        time.sleep(1)
