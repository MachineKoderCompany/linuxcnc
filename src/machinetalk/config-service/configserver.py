...
from machinetalk_lib import service, config
...
class ConfigServer(object):
        ...
        for rootdir in self.appDirs:
            for root, _, files in os.walk(rootdir):
                if 'description.ini' in files:
                    ...
                    logger.debug("name: " + cfg.get('Default', 'name'))
                    logger.debug("description: " + cfg.get('Default', 'description'))
                    logger.debug("type: " + cfg.get('Default', 'type'))
    ...
    def send_msg(self, dest, type_):
        ...
        logger.debug("send_msg " + str(self.tx))
        self.tx.Clear()
    ...
    def retrieve_app(self, origin, name):
        logger.debug("retrieve app " + name)
        app = self.tx.app.add()
        app.name = name
        app.description = self.cfg.get(name, 'description')
        app.type = self.type_to_pb(self.cfg.get(name, 'type'))
    ...
    def process(self, s):
        ...
        logger.debug("process called, id: %s" % identity)
...
def main():
    ...
    parser.add_argument(
        '-n', '--name', help='Name of the machine', default="Machinetalk"
    )
    parser.add_argument(
        '-s',
        '--suppress_ip',
    ...
    logger.debug("announcing configserver")
